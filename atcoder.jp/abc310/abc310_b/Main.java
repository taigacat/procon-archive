...
public class Main {
  ...
  void solve() {
    ...
    for (int i = 0; i < n; i++) {
      ...
      var functionCount = in.nextInt();
      var functions = in.nextIntArray(functionCount);
      items.add(new Item(price, Arrays.stream(functions).boxed().collect(Collectors.toSet())));
    }
    ...
    for (var item : items) {
      for (var item2 : items) {
        ...
        if (item.getPrice() >= item2.getPrice()
            && item2.getFunctions().containsAll(item.getFunctions())
            && (item.getPrice() > item2.getPrice() || item2.getFunctions().size() > item.getFunctions().size())) {
          result = true;
          break;
        }
      }
...
class Item {
  ...
  private final Set<Integer> functions;

  public Item(int price, Set<Integer> functions) {
    this.price = price;
