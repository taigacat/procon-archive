...
public class Main {
  ...
  void solve() {
    int n = in.nextInt();
    int teika = in.nextInt();
    int discounted = in.nextInt();

    int[] foods = in.nextIntArray(n);
    int food = Arrays.stream(foods).min().orElse(0);

    out.println(Math.min(teika, discounted + food));
  }
