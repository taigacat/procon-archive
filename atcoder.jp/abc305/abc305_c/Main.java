...
public class Main {
  ...
  void solve() {
    ...
    Set<Integer> cookieExistColumn = new HashSet<>();
    Set<Integer> cookieExistRow = new HashSet<>();
    boolean[][] cookieExist = new boolean[height][width];

    for(int i = 0; i < height; i++) {
      char[] line = in.nextCharArray();
      for(int j = 0; j < width; j++) {
        if(line[j] == '#') {
          cookieExistColumn.add(j);
          cookieExistRow.add(i);
          cookieExist[i][j] = true;
        } else {
          cookieExist[i][j] = false;
        }
      }
    }

    for (int i = 0; i < cookieExist.length; i++) {
      boolean[] row = cookieExist[i];
      if (cookieExistRow.contains(i)) {
        for (int j = 0; j < row.length; j++) {
          if (cookieExistColumn.contains(j) && !row[j]) {
            out.println((i + 1) + " " + (j + 1));
          }
        }
      }
