...
public class Main {
  ...
  void solve() {
    int types = in.nextInt(); // N
    int max = in.nextInt(); // K

    List<DragInfo> list = new ArrayList<>(types);
    for (int i = 0; i < types; i++) {
      int period = in.nextInt(); // ai
      int amount = in.nextInt(); // bi
      list.add(new DragInfo(period, amount));
    }

    list.sort((o1, o2) -> o2.period - o1.period);
...
class DragInfo implements Comparable<DragInfo> {

  int period;
  int amount;

  public DragInfo(int period, int amount) {
    this.period = period;
    this.amount = amount;
  }
  ...
  @Override
  public int compareTo(DragInfo o) {
