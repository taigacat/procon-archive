...
public class Main {
  ...
  void solve() {
    int n = in.nextInt();
    List<Long> walkInWeek = new ArrayList<>();
    long walkInThisWeek = 0;
    for (int i = 0; i < n * 7; i++) {
      walkInThisWeek += in.nextInt();
      if (i % 7 == 6) {
        walkInWeek.add(walkInThisWeek);
        walkInThisWeek = 0;
      }
    }

    out.println(walkInWeek.stream().map(Object::toString).collect(Collectors.joining(" ")));
